                           time             space        stable(稳定：如果a原本在b前面，而a=b，排序之后a仍然在b的前面。)
1. 插入排序 Insert sort   O(n)~O(n^2)         O(1)           Y 对于未排序数据，在已排序序列中从后向前扫描，找到相应位置并插入
2. 选择排序 Select sort   O(n^2)              O(1)           N 每次选最小/大的放到前面去
3. 堆排序 heap sort       O(nlogn)            O(1)           N 堆积是一个近似完全二叉树的结构，并同时满足堆积的性质：即子结点的键值或索引总是小于（或者大于）它的父节点
4. 冒泡排序 Bubble sort   O(n)~O(n^2)         O(1)           Y 它重复地走访过要排序的数列，一次比较两个元素，如果它们的顺序错误就把它们交换过来
5. 快速排序 Quick sort    O(nlogn)~O(n^2)     O(nlogn)       N 通过一趟排序将待排记录分隔成独立的两部分，其中一部分记录的关键字均比另一部分的关键字小，则可分别对这两部分记录继续进行排序，以达到整个序列有序。
6. 归并排序 Merge sort    O(nlogn)            O(n)           Y 将已有序的子序列合并，得到完全有序的序列；即先使每个子序列有序，再使子序列段间有序
